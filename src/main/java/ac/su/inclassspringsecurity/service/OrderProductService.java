...
public class OrderProductService {
    ...
    // TransactionTemplate를 사용하여 Transaction을 직접 제어할 수 있음
    private final TransactionTemplate transactionTemplate;

    // 2) EntityManager 주입
    // EntityManager를 사용하여 트랜젝션을 직접 제어할 수 있음
    // 현재 Bean 내에서 동일한 EntityManager 객체로 Transaction을 제어하는 단위가 형성됨
    // => DB 접근을 Transaction의 요건에 맞춰서 제어할 수 있는 단일 Context로 작용
    // => ACID 요건 중 Isolation 요건을 갖추려면 이렇게 단일 Context로 작용하는 EntityManager가 필요
    @PersistenceContext
    private EntityManager entityManager; // 현재 객체에 주입되는 Repository들과 Binding되는 효과
    ...
    public void createDummyOrderProductWithInlineTransaction(){
        transactionTemplate.execute(status -> {
            try {
                ...
                // 연관 관계 뿐만 아니라, Atomic 하게 다루고 싶은 작업은 전부 트랜젝션으로 묶어서 처리
